...
export async function teacherView(app) {
  ...
  function cargarPreguntaEnFormulario(pregunta) {
    ...
    correctaInput.value = Number.isInteger(pregunta.correcta) ? pregunta.correcta : "";
  ...
  saveLocalButton.onclick = async () => {
    ...
    if (!preguntasActualizadas.length) {
      alert("No hay preguntas para guardar localmente.");
      return;
    }

    saveLocalQuestions(preguntasActualizadas);
    alert(`✅ ${preguntasActualizadas.length} preguntas guardadas para jugar offline`);
  };
  ...
  const preguntasOrdenadas = [...preguntas].sort((a, b) => {
    const nivelA = Number(a.nivel);
    const nivelB = Number(b.nivel);
    const valorA = Number.isFinite(nivelA) ? nivelA : Number.MAX_SAFE_INTEGER;
    const valorB = Number.isFinite(nivelB) ? nivelB : Number.MAX_SAFE_INTEGER;
